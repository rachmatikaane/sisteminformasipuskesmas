...
class JadwalController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				jadwal.id LIKE ? OR 
				jadwal.hari LIKE ? OR 
				jadwal.jam_mulai LIKE ? OR 
				jadwal.jam_selesai LIKE ? OR 
				jadwal.libur LIKE ? OR 
				jadwal.created_at LIKE ? OR 
				jadwal.updated_at LIKE ?
			)";
			$search_params = array(
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("jadwal.id", ORDER_TYPE);
		}
		if($fieldname){
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("jadwal.id", $rec_id);; //select record based on primary key
		}
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("hari","jam_mulai","jam_selesai","libur");
			...
			$this->sanitize_array = array(
				...
				'jam_mulai' => 'sanitize_string',
				'jam_selesai' => 'sanitize_string',
				'libur' => 'sanitize_string',
			);
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","hari","jam_mulai","jam_selesai","libur");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'hari' => 'required',
	...
	function delete($rec_id = null){
		...
		$db->where("jadwal.id", $arr_rec_id, "in");
