...
class Rekam_medisController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				rekam_medis.id LIKE ? OR 
				rekam_medis.tanggal LIKE ? OR 
				rekam_medis.anamnesis LIKE ? OR 
				rekam_medis.hasil_pemeriksaan LIKE ? OR 
				rekam_medis.created_at LIKE ? OR 
				rekam_medis.updated_at LIKE ?
			)";
			$search_params = array(
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("rekam_medis.id", ORDER_TYPE);
		}
		if($fieldname){
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("rekam_medis.id", $rec_id);; //select record based on primary key
		}
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("tanggal","anamnesis","hasil_pemeriksaan");
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","tanggal","anamnesis","hasil_pemeriksaan");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'tanggal' => 'required',
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("rekam_medis.id", $rec_id);;
	...
	function delete($rec_id = null){
		...
		$db->where("rekam_medis.id", $arr_rec_id, "in");
